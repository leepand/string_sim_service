...
class TreeModel:
    ...
    def insert(self, brand_name,status,brand_type):        
        ...
        self.tree.add(sim_model.resub_string(brand_name+'_'+brand_type,resub_list))
    ...
    def query(self,new_brand_name,N):
        ...
        for dist,brand in list(set(sorted(self.tree.find(new_brand_name, 1))))[:N]:
            ...
            #    continue
            query_condition=db.table(brand_TABLE).search((Model.brand_index == brand) & (Model.brand_index_info.status=='active'))
...
def update_model():
    ...
    if request.headers['Content-Type'] == 'text/plain':
        ...
        tree=model.insert(brand,status,brand_type)
        #s['tree']=tree
        #s.close()
